## PPO

>整体一句话（先给结论）
- 在 RLHF 中，PPO 做的事只有一件：
- 用 Advantage 调整每个 token 的生成概率，
- 同时通过 KL 约束保证模型不偏离参考模型太远。

#### Step 1：Actor 生成 token（状态–动作）

```
天空 ｜ 是 ｜ 蔚蓝色 ｜ 的
```
在第 $t$ 步：
- 状态 $S_t$ ：prompt + 已生成 tokens
- 动作 $A_t$ ：下一个 token

Actor（policy）给出：

$$
P\left(A_t \mid S_t\right)
$$


同时我们有一个冻结的 reference policy：

$$
P_{\mathrm{ref}}\left(A_t \mid S_t\right)
$$

#### Step 2：构造 token-level reward
##### KL 即时惩罚（每一步都有）

$$
r_t^{K L}=-k_{k l} \cdot \log \frac{P\left(A_t \mid S_t\right)}{P_{\mathrm{ref}}\left(A_t \mid S_t\right)}
$$


含义：每生成一个 token，只要偏离参考模型，就扣分

##### 终止奖励（只在最后一步）
在 $t=T$ ：

$$
r_T=r_T^{K L}+r_{\mathrm{RM}}
$$

也就是：
- KL 惩罚
- Reward Model 对整句的偏好分数

至此，我们得到了：

$$
\left\{r_1, r_2, \ldots, r_T\right\}
$$

#### Step 3：Critic 评估每个状态（你第二张图）

Critic model 对每个状态输出一个值：

$$
V_\theta\left(S_t\right)
$$

| token | (V(S_t)) |
| ----- | -------- |
| 天空    | -0.84    |
| 是     | 0.88     |
| 蔚蓝色   | 1.96     |
| 的     | 3.48     |

含义：从当前 token 开始，预计最终能拿到多少总 reward

#### Step 4：TD 误差
对每一步计算：

$$
\delta_t=r_t+\gamma V\left(S_{t+1}\right)-V\left(S_t\right)
$$


直觉解释：
- ＂真实发生的＋下一步预期＂
- 和＂critic 原本的预期＂差多少

这是 Advantage 的最小组成单位。

#### Step 5：GAE → Advantage

$$
A_t=\sum_{b=0}^{\infty}(\gamma \lambda)^b \delta_{t+b}
$$


作用：
- 把后面 token 的好坏
- 按距离衰减
- 分摊给当前 token

📌 这一步把 token 序列奖励
变成了 token 序列 Advantage

#### Step 6：PPO 更新 policy（最终目标）
定义概率比：

$$
\rho_t=\frac{P\left(A_t \mid S_t\right)}{P_{\text {old }}\left(A_t \mid S_t\right)}
$$


PPO 的策略损失（TRL／OpenAI 风格）：

$$
C_{\text {clip }}=-\mathbb{E}_t\left[\min \left(\rho_t A_t, \operatorname{clip}\left(\rho_t, 1-\epsilon, 1+\epsilon\right) A_t\right)\right]
$$


含义：
- 如果这次更新让 token 概率变太多
- 梯度直接被截断


### Critic ＆Entropy（完整 loss）

$$
\mathcal{L}=C_{\text {clip }}+c_V C_V+C_{\text {ent }}
$$


其中：
- $C_V=\frac{1}{2}\left(V\left(S_t\right)-\hat{G}_t\right)^2$
- $C_V$（大写 C）: Value loss 本身,一个“损失项” 
- $c_V$（小写 c）: Value loss 的权重系数, 一个“超参数” 
- $C_{\text {ent }}$ ：防止策略塌缩

#### 1️⃣ Value loss 本身：$C_V$

$$
C_V=\frac{1}{2} \mathbb{E}_t\left[\left(V_\theta\left(s_t\right)-\hat{G}_t\right)^2\right]
$$


含义：critic 预测的 value 和真实 return 的误差
这是一个标量损失函数。

#### 2️⃣ 权重系数：$c_V$
- 一个人为设定的超参数
- 控制 critic 更新在总 loss 中有多重要

常见取值（经验）：
- TRL／OpenAI：$c_V＝ 0.5$
- 有时： $0.1 \sim 1.0$
...
- 尝试用 `SVD` 对一个随机 `W` 进行压缩，并重构低秩近似结果
- 比较 `full W` 与 `LoRA` 注入 `W_lora` 对于相同输入的输出差异

### 5 本课小结

- `LoRA` 基于经典的线性代数低秩逼近思想构建
- 插入点设计对模型效果与效率至关重要
- `NumPy` 实现帮助理解权重扰动方式与压缩效果

### 6 课后任务

- 阅读相关资料：SVD 与低秩逼近在图像压缩与语言建模中的应用
- 拓展练习：将 `rank` 升为 `2、4、8`，观察 `delta_W` 与性能关系
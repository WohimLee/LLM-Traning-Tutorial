# prefix_mlp_tuning_gpt_poetry_full.py
import os
import math
import random
import numpy as np
...
'''
可以调的 3 个关键超参（经验值）
- prefix_len：8 / 16 / 32（越大越强但更慢）
- prefix_mlp_hidden：256 / 512 / 1024（越大表达越强）
- train_cls_head=True：一般建议开（更稳、更快看到效果）
'''
...
class Poetry(Dataset):
    ...
    Causal LM:
      ...
      labels:    shift-left + eos, pad on pad positions
    """
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, q, k, v, key_padding_mask=None, prefix_k=None, prefix_v=None):
        ...
        # mask out pad queries too
        q_valid = key_padding_mask[:, :Tq].unsqueeze(1).unsqueeze(-1)  # (B,1,Tq,1)
        allow = allow & q_valid
...
# -----------------------
# Prefix-MLP module (shared tokens, per-layer MLP)
# -----------------------
class PrefixMLP(nn.Module):
    """
    Learn prefix tokens: (P, D)
    For each layer l, an MLP maps token embeddings -> (2, H, P, d_k)  (K and V)

    Implementation:
      prefix_tokens: nn.Parameter(P, D)
      layer_mlps: ModuleList of MLPs
        each MLP: D -> hidden -> (2*H*d_k)
      We apply MLP to each prefix token position independently.
    """
    def __init__(self, num_layers, d_model, num_heads, prefix_len, mlp_hidden=512, dropout=0.0):
        super().__init__()
        assert d_model % num_heads == 0
        ...
        self.layer_mlps = nn.ModuleList([
            nn.Sequential(
                ...
                nn.Tanh(),  # 常见用 tanh
                nn.Linear(mlp_hidden, 2 * num_heads * self.d_k),
            )
...
class GPTBlockPrefixMLP(nn.Module):
    """
    Block takes prefix_k/prefix_v for THIS layer from outside (computed once per forward).
    """
    def __init__(self, d_model, num_heads, dff, dropout):
        super().__init__()
        self.mha = MultiheadAttention(d_model, num_heads, dropout)
...
class GPTPrefixMLPModel(nn.Module):
    """
    Same base architecture, but attention uses Prefix-MLP-generated KV per layer.
    """
    def __init__(self, vocab_size, d_model, max_length, num_heads, dff, dropout, N, prefix_len, mlp_hidden=512):
        super().__init__()
        self.N = N
        self.emb = nn.Embedding(vocab_size, d_model)
        self.pos = nn.Embedding(max_length, d_model)
        self.blocks = nn.ModuleList([GPTBlockPrefixMLP(d_model, num_heads, dff, dropout) for _ in range(N)])
        self.cls = nn.Linear(d_model, vocab_size)
        self.register_buffer("pos_range", torch.arange(max_length).view(1, -1), persistent=False)
    ...
    def forward(self, input_ids, key_padding_mask=None):
        ...
        # compute ALL layer prefixes once
        prefixes = self.prefix_mlp(batch_size=B, device=x.device, dtype=x.dtype)
...
# -----------------------
# Main (Two-stage: Base -> Prefix-MLP tuning)
# -----------------------
if __name__ == "__main__":
    ...
    prefix_mlp_ckpt = "prefix_mlp_tuned_ckpt.pt"
    ...
    # ==========================================================
    # Stage 2: Prefix-MLP Tuning (freeze base, train prefix_mlp (+ optional cls))
    # ==========================================================
    print("\n=== Stage 2: PREFIX-MLP TUNING (freeze base) ===")
    prefix_model = GPTPrefixMLPModel(
        vocab_size, d_model, max_length, num_heads, dff, dropout, N,
        prefix_len=prefix_len,
        mlp_hidden=prefix_mlp_hidden
    )

    ckpt = torch.load(base_ckpt, map_location="cpu")
    missing, unexpected = prefix_model.load_state_dict(ckpt["model_state"], strict=False)
    print("Loaded base into prefix-mlp model (strict=False).")
    print("Missing keys (expected: prefix_mlp params):", len(missing))
    ...
    # Unfreeze only prefix_mlp params
    for name, p in prefix_model.named_parameters():
        if name.startswith("prefix_mlp."):
    ...
    train_loop(
        ...
        stage_name="PREFIX-MLP",
        dataset=dataset,
        tokenizer=tokenizer,
        max_length=max_length
    )

    torch.save({"model_state": prefix_model.state_dict()}, prefix_mlp_ckpt)
    print(f"Saved prefix-mlp tuned checkpoint: {prefix_mlp_ckpt}")
